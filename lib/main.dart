...
class GoogleSearchService {
  ...
  Future<String?> search(String query) async {
    try {
      debugPrint("🔍 Searching Google for: $query");
      ...
      if (response.statusCode == 200) {
        ...
        if (data['items'] == null || (data['items'] as List).isEmpty) {
          debugPrint("⚠️ No search results found.");
          return null;
        }
        ...
        debugPrint("❌ Google Search Quota Exceeded");
        return "Google Search daily limit reached. Unable to fetch real-time data.";
      } else {
        debugPrint(
            "❌ Google Search Error: ${response.statusCode} - ${response.body}");
        return null;
      }
    } catch (e) {
...
class VoiceController extends GetxController {
  ...
  String _applySpeakingMode(SpeakingMode mode, String text,
    ...
    if (mode == SpeakingMode.story) {
      ...
      debugPrint("🎭 Story Mode Activated - Immersive pacing enabled");
  ...
  String _normalizeHindiForTTS(String text) {
    ...
    final Map<String, String> criticalWords = {
      'main': 'mai', // "main" → "mai" (not "mayne")
      'hain': 'hai', // "hain" → "hai" (soft nasal, not "hane")
      'kahin': 'kaheen', // "kahin" → "kaheen" (stretch middle vowel)
      'tumhein': 'tumhe', // "tumhein" → "tumhe" (soft ending)
      'rahein': 'rahe', // "rahein" → "rahe" (soft ending)
      'chahein': 'chahe', // "chahein" → "chahe"
      'jayein': 'jaye', // "jayein" → "jaye"
      'aayein': 'aaye', // "aayein" → "aaye"
    };
    ...
    // Examples: jeelein → jeele, karein → kare, chalein → chale
    ...
    // Examples: baatein → baate, raastein → raaste, kahen → kahe
    result = result.replaceAllMapped(
    ...
    // Examples: logon → logo, gharon → gharo, raaton → raato
    result = result.replaceAllMapped(
    ...
    // RULE 5: Improved nasal sound handling (un/oon/uun → oo or o)
    // Examples: hoon → hoo, kyun → kyoo (soft nasal pause, not harsh consonant)
    result = result.replaceAllMapped(
  ...
  /// Examples: "S H O U R A V" → "SHOURAV", "K U M A R" → "KUMAR"
  String _fixSpelledOutNames(String text) {
    ...
    if (result != text) {
      debugPrint("🔤 Fixed spelled names: $text → $result");
    }
...
• While speaking, you must NOT use or reference:
...
• Do not include formatting cues

LISTENING-FIRST DELIVERY:
• Assume the user is primarily listening, not reading
• Keep responses calm, uninterrupted, and easy to follow
• Deliver one clear thought at a time
• Avoid rapid topic switching
• Leave gentle pauses in meaning

COGNITIVE LOAD AWARENESS:
• The user is thinking about their next question while listening
• Do not overwhelm with excessive details
• Prefer clarity over completeness
• If more depth is possible, offer it briefly instead of delivering immediately

DEVELOPER IDENTITY & CONTEXT:
• Developer Name: SHOURAV
• Team: CTJ TEAM
• Launch Date: JANUARY 2026
• City: JAMSHEDPUR
• Website: codingtutorialsjamshedpur.fun
• If asked about any of the above, provide the specific details.
• This is an ALPHA VERSION for testing.

CRITICAL NAME HANDLING RULES (FOR TTS):
• When mentioning names (user name, developer name, etc.), ALWAYS write them as SINGLE WORDS
• Examples: 
  - Write: "SHOURAV" NOT "S H O U R A V"
  - Write: "KUMAR" NOT "K U M A R"
  - Write: "PRIYA" NOT "P R I Y A"
• For full names with spaces, only mention LAST NAME in casual speech
• Example: If user name is "SHOURAV KUMAR", refer as "KUMAR" or "SHOURAV" alone
• This ensures TTS speaks names naturally, not letter-by-letter
• In AppBar display with limited space: show only first name (e.g., "SHOURAV..")

CURRENT CONTEXT:
• Date: $dayString, $dateString
• Time: $timeString (IST)
• User Name: ${userName.value}
• User Title: ${userTitle.value}
• If the user asks to change their name, you MUST output exactly: [UPDATE_NAME: NewName]
• Do not output the [UPDATE_NAME] tag if you are just addressing the user, only if they explicitly ask to change their name.

SETTINGS CONTROL:
• If user asks to change language (Hinglish/English) or voice (Male/Female), output exactly: [SET_VOICE: Language-Gender]
• Examples: [SET_VOICE: Hinglish-Male], [SET_VOICE: Hinglish-Female], [SET_VOICE: English-Female]
• Hinglish is mix of Hindi and English words
• Only output this tag if the user explicitly asks to change settings.
''';
...
• Use respectful, simple Hindi
• Avoid heavy Sanskrit or complex vocabulary
• Keep tone friendly and human, not formal or preachy
• When saying names in Hindi: जीवन (Jeevan) NOT ज ी व न
''';
    } else {
      basePrompt += '''\nLANGUAGE-SPECIFIC TONE (English US):
• Use warm, conversational, natural American cadence
• Be friendly and approachable
• Say names as complete words: "Shourav" NOT "S-H-O-U-R-A-V"
''';
    }

    basePrompt += '''\nABSOLUTE PRIORITY:
• Listening experience always comes before cleverness, verbosity, or stylistic flair
• If there is a conflict, simplify
• Names must ALWAYS be pronounced as complete words by TTS, never letter-by-letter
''';
  ...
  */
    ...
    try {
      ...
      if (availableVoices != null) {
        ...
        var mappedVoices = filteredVoices.map((v) {
          ...
          String displayName = "$language – $gender";
  ...
  void applyPersonaEffects(String persona) {
    ...
    // 🔹 PERSONA-AWARE LANGUAGE TONE CONSTRAINTS
  ...
  Future<void> startListening() async {
    ...
    debugPrint("🎤 Starting listening session...");
    ...
    if (userMsg.toLowerCase().contains('change name') ||
      ...
      if (newTitle != null && newName != null && newName.isNotEmpty) {
        ...
        final confirmMessage =
            "Bilkul! Main aapko $newTitle $newName ke naam se bulaunga ab! 👋";
  ...
  Future<void> speak(String text) async {
    ...
    // 🔹 DETECT SPEAKING MODE (Story vs Utility)
    ...
    // 🔹 INDIAN ENGLISH TUNING - Reduce US-style dramatic emphasis
    if (locale == "en-US") {
    ...
    // 🔹 AUDIO HYGIENE - Wait for sound effects to complete before speaking
    ...
    // 🔹 FIX SPELLED-OUT NAMES before normalization
    String textWithFixedNames = _fixSpelledOutNames(cleanText);

    // 🔹 APPLY HINDI TTS NORMALIZATION before speaking
    String textAfterNormalization = _normalizeHindiForTTS(textWithFixedNames);

    // 🔹 APPLY SPEAKING MODE TEXT PREPROCESSING (Story mode adds pauses)
  ...
  void _handleTimeUp() {
    addMessage("assistant", "⏱️ Time's up! Let me share how we did...");
...
${getListeningFirstPrompt()}''';
  ...
  String _generateGameSummary() {
    switch (activeGame.value) {
      case '20 Questions':
        ...
        return "In 20 Questions, I asked you $questions questions. That was challenging! 🤔";

      case 'Roast Battle':
        int roasts = gameState['roastCount'] ?? 0;
        return "We had $roasts amazing roasts back and forth! You're hilarious! 🔥";

      case 'Compliment Generator':
        int compliments = gameState['complimentCount'] ?? 0;
        return "I gave you $compliments wonderful compliments. Remember, you deserve them all! 💫";

      case 'Story Time':
        int segments = gameState['storySegments'] ?? 0;
        return "We went through $segments story segments on your epic adventure! 📖";

      case 'Debate Mode':
        int points = gameState['debatePoints'] ?? 0;
        return "We made $points debate points each. That was intellectually stimulating! 🧠";
      ...
      case 'Trivia Challenge':
        ...
        if (total > 0) {
          ...
          return "You got $correct out of $total correct! That's $percentage%! Great job! 🎓";
        }
        return "Great trivia session! You scored $correct points! 🎓";

      case 'Astro Talk':
        int predictions = gameState['predictionsGiven'] ?? 0;
        return "I gave you $predictions astrological readings. May the stars guide you! 🌟";

      default:
        return "Great game session! You scored ${gameScore.value} points! 🎮";
    }
  }
  ...
  void endGame({String? reason}) async {
    ...
    if (activeGame.value != null && gameScore.value > 0) {
      ...
      if (highScore == gameScore.value && gameScore.value > 0) {
        ...
        addMessage("assistant", "🏆 NEW HIGH SCORE! 🏆");
      }
    }
  ...
  String _getGameEndMessage() {
    ...
    switch (activeGame.value) {
      ...
      case 'Roast Battle':
        int roasts = gameState['roastCount'] ?? 0;
        return roasts > 5
            ? "🔥 LEGENDARY roast battle! You had me dying! $roasts roasts exchanged!"
            : roasts > 3
      ...
      case 'Compliment Generator':
        ...
        return compliments > 5
            ? "💫 Wow! I gave you $compliments amazing compliments! You're incredible!"
            : "I shared $compliments wonderful thoughts about you. You deserve them all!";

      case 'Story Time':
        int segments = gameState['storySegments'] ?? 0;
        return "What an adventure! We went through $segments story segments and created an epic tale together! 📖";

      case 'Debate Mode':
        int points = gameState['debatePoints'] ?? 0;
        return points > 5
            ? "🧠 Incredible debate! $points strong arguments made! You think critically!"
      ...
      case 'Trivia Challenge':
        ...
        if (total > 0) {
          int percentage = ((correct / total) * 100).toInt();
          if (percentage == 100) {
            return "🏆 PERFECT SCORE! You got all $total questions correct! You're a trivia master!";
      ...
      case 'Astro Talk':
        int predictions = gameState['predictionsGiven'] ?? 0;
        return "🌟 I provided $predictions astrological readings for you. May the cosmic energy guide your path!";
  ...
  String _generateAstrologicalPrediction() {
    ...
    prediction.writeln("🌟 Astrological Reading for $birthPlace 🌟\n");
                        ...
                        child: Row(
                          children: [
                            ...
                            GestureDetector(
                              ...
                              onLongPressStart: (_) {
                                debugPrint(
                                    "🎤 Mic pressed - starting recording");
                              ...
                              onLongPressEnd: (_) {
                                debugPrint(
                                    "🎤 Mic released - stopping recording");
                                if (controller.isListening.value) {
...
class NaamJapController extends GetxController {
  ...
  void _nextChant() async {
    ...
    currentCount.value++;
    await tts.setSpeechRate(speed.value * 0.5); // Adjust base rate
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Text(session.mantra,
                    style: const TextStyle(color: Colors.white)),
                subtitle: Text(
                  "${DateFormat.yMMMd().add_jm().format(session.timestamp)} • ${session.durationSeconds}s",
